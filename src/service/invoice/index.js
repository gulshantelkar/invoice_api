...
class InvoiceService {
    ...
    getOverDueInvoices = async () => {
        ...
        const invoices = await Invoice.find({due: {$lt: currentUnixValue} , due_alert_sent: false});
        return invoices;
    }
    ...
    updateDueAlert = async (invoices) => {
        for(let i=0; i<invoices.length; i++) {
            ...
            console.log('Sent mail to : ',invoices[i]._id,invoices[i].due_alert_sent);
        }
